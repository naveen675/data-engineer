...
def send_email(sender_email, sender_password, receiver_email, subject, status, file_path, number_of_rows_processed):
    ...
    try:
        ...
        if status == 'Duplicate':
            ...
            subject = f"File Already Processed : {file_name}"
            ...
            subject = f'New file received to bucket :{file_name}'
...
def process_start(event, context):
    ...
    try:
        ...
        # read_file_from_storage(bucket_name, file_name,storage_df)
        ...
        if "Can only compare identically-labeled" in str(e):
            ...
            print("Missing Mandatory Column")
            logger.log_text(f"Missing Mandatory Column ID")
            move_file(file_name, "Insufficient_Data")
            end_time = datetime.now()
            update_tracker_table(file_name, failed, "Mandatory Column is missing in file",
                                 start_time, end_time, file_upload, duration, process, 'inactive')
